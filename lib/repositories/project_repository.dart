...
abstract class ProjectRepository {
  ...
  Future<void> deleteTaskFromProject({required String projectId, required String taskId});
}
...
class ProjectRepositoryImpl implements ProjectRepository {
  ...
  @override
  Future<Project> addTaskToProject({required String projectId, required Task task}) async {
    final project = await projectService.addTaskToProject(projectId, taskMapper.toDTO(task));
    return projectMapper.fromDTO(project);
  }

  @override
  Future<Project> createProject({required Project project}) async {
    final newProject = await projectService.createProject(projectMapper.toDTO(project));
  ...
  Future<Project> uploadProjectImageCover({required String id, required File imageCover}) async {
    final newProject = await projectService.uploadProjectImageCover(id, imageCover);
    return projectMapper.fromDTO(newProject);
  }

  @override
  Future<Project> deleteProjectImageCover({required String id}) async {
    final project = await projectService.deleteProjectImageCover(id);
    return projectMapper.fromDTO(project);
  }
  ...
  @override
  Future<void> deleteTaskFromProject({required String projectId, required String taskId}) async {
    await projectService.deleteTaskFromProject(projectId, taskId);
  }

  @override
  Future<List<Project>> getProjectsByUser() async {
    final projects = await projectService.getProjectsByUser();
    return projects.map((p) => projectMapper.fromDTO(p)).toList();
  }

  @override
  Future<Project> updateProjectById({required String id, required Project project}) async {
    final updatedProject = await projectService.updateProjectById(id, projectMapper.toDTO(project));
  ...
  Future<Project> updateTaskInProject(
    ...
    final updatedProject =
        await projectService.updateTaskInProject(projectId, taskId, taskMapper.toDTO(task));
    return projectMapper.fromDTO(updatedProject);
  }
}
